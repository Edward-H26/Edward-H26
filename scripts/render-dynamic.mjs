// Cards rendered from live GitHub data by the profile-cards workflow.
import { MONO, cardFrame, escape, round, svgDocument, textWidth } from "./svg.mjs"
...
export function renderStats(stats, theme) {
  ...
  const ring = `<g transform="translate(120 122)"><circle r="54" fill="none" stroke="${theme.border}" stroke-width="8"/><circle r="54" fill="none" stroke="url(#stats-ring)" stroke-width="8" stroke-linecap="round" stroke-dasharray="${ringLength}" stroke-dashoffset="${ringLength}" transform="rotate(-90)"><animate attributeName="stroke-dashoffset" from="${ringLength}" to="${round(ringLength * 0.12)}" dur="1.8s" fill="freeze"/></circle><text y="-2" text-anchor="middle" font-size="26" font-weight="800" fill="${theme.text}">${escape(compact(stats.total))}</text><text y="17" text-anchor="middle" font-size="9" font-weight="600" letter-spacing="0.6" fill="${theme.muted}">CONTRIBUTIONS</text></g>`
  ...
  const tiles = [
    ...
    .map(([label, value, color], i) => {
      ...
      return `<g transform="translate(${x} ${y})" opacity="0"><animate attributeName="opacity" from="0" to="1" begin="${round(0.15 + i * 0.08)}s" dur="0.5s" fill="freeze"/><rect width="106" height="78" rx="14" fill="${color}" fill-opacity="0.09" stroke="${color}" stroke-opacity="0.35"/><text x="14" y="40" font-size="26" font-weight="800" fill="${theme.text}">${escape(compact(value))}</text><text x="14" y="62" font-size="12" fill="${theme.muted}">${escape(label)}</text></g>`
    })
    .join("")
  ...
  const footer = `<text x="${WIDTH - 48}" y="${height - 28}" text-anchor="end" font-size="11" font-family="${MONO}" fill="${theme.faint}">@${escape(stats.login)} · updated ${escape(stats.updated)}</text>`
  const defs = `${card[0]}<linearGradient id="stats-ring" x1="0" x2="1"><stop offset="0" stop-color="${theme.accent2}"/><stop offset="1" stop-color="${theme.accent}"/></linearGradient>`
  return svgDocument({ width: WIDTH, height, title: `GitHub activity of ${stats.name}`, theme, defs, body: [card[1], labels, ring, streak, tiles, languages, heatmap(theme, stats.weeks, 300, 262), footer].join("\n") })
}
...
export function renderActivity(stats, theme) {
  ...
  const list = rows
    .map((item, i) => {
      ...
      return `<g opacity="0"><animate attributeName="opacity" from="0" to="1" begin="${round(0.2 + i * 0.18)}s" dur="0.45s" fill="freeze"/><animateTransform attributeName="transform" type="translate" values="-16 0;0 0" begin="${round(0.2 + i * 0.18)}s" dur="0.45s" fill="freeze"/><circle cx="66" cy="${y - 5}" r="6" fill="${color}"/><line x1="66" y1="${y + 9}" x2="66" y2="${y + rowHeight - 12}" stroke="${theme.border}" stroke-dasharray="2 4"/><text x="${textX}" y="${y}" font-size="15" fill="${theme.text}">${escape(item.text)}</text><text x="${repoX}" y="${y}" font-size="14" font-family="${MONO}" font-weight="600" fill="${color}">${escape(item.repo)}</text><text x="${WIDTH - 52}" y="${y}" text-anchor="end" font-size="12" font-family="${MONO}" fill="${theme.faint}">${escape(item.when)}</text></g>`
    })
    .join("\n")
  const header = `<text x="60" y="54" font-size="12" font-weight="600" letter-spacing="1.2" fill="${theme.muted}">RECENT PUBLIC ACTIVITY</text><text x="${WIDTH - 52}" y="54" text-anchor="end" font-size="11" font-family="${MONO}" fill="${theme.faint}">updated ${escape(stats.updated)}</text>`
  return svgDocument({ width: WIDTH, height, title: `Recent GitHub activity of ${stats.name}`, theme, defs: card[0], body: [card[1], header, list].join("\n") })
}
